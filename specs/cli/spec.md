---
status: approved
summary: Specification for the `cxg lint` workflow, a non-interactive CLI path that lints and formats contextual commit messages and outputs them to stdout for AI agents.
---

## Context and Goals

- Context: Contextual commits extend Conventional Commits by adding action lines (intent, decision, rejected, constraint, learned) to the commit body to preserve "why the code was written." cxg is a CLI tool that lints and formats these messages.
- Goal: `cxg lint` lints and formats messages, outputs them to stdout, and lets agents pipe the result to git. It can also be used as a `commit-msg` hook. Unlike czg (an interactive git wrapper), this `lint` workflow is a non-interactive message processor. The main command in this spec is `cxg lint`; auto-fix is available via the `--fix` flag. The separate `cxg commit` workflow is specified in `specs/commit/spec.md`.

## User Scenarios and Testing

### User Story 1 - Lint a commit message and pipe it to git (Priority: P1)
An AI agent wants to lint a message and, if it passes linting, stream it to stdout and pass it to `git commit -F -`.
...
Register cxg as a `commit-msg` hook to automatically lint messages on `git commit`.
...
Acceptance Scenarios:
1. `cxg lint <filepath>` reads the message from the file and lints it. Exits 0 if it passes linting; outputs an error to stderr and exits 1 if it fails linting.
...
3. If the combined message passes linting, it is written to stdout.

### User Story 4 - Auto-fix a message with --fix before linting (Priority: P2)
An AI agent wants to auto-fix a rough generated message, lint it, and pass it to git.

Why this priority: Agent-generated messages often have missing blank lines or trailing spaces.
Independent Test: `cxg lint --fix -m "feat(auth): add login\nintent(auth): ..."` outputs a blank-line-normalized, linted message.
Acceptance Scenarios:
1. If the gap between the subject and body is 0 or 2+ lines, normalize it to exactly one blank line, lint it, and output to stdout if it passes linting.
...
- `cxg lint --fix` auto-fixes first, then lints; only lint-clean output is written to stdout.

## Requirements

### Functional Requirements

- FR-001: `cxg lint` accepts a message from `-m` (multiple allowed), stdin, or a file path argument, and lints it.
...
- FR-005: The subject line is linted against Conventional Commits format. Fixed type list: feat, fix, refactor, perf, test, docs, style, build, ci, chore, revert.
- FR-006: Action lines are linted as `action-type(scope): description`. Valid types: intent, decision, rejected, constraint, learned.
- FR-007: Exit code is 0 on success and 1 on lint error.
...
- FR-010: The `--fix` flag auto-fixes the message before linting. If it still fails linting after fixing, output to stderr and exit 1. Fix operations: (1) normalize the subject-body gap to exactly one blank line (add if missing, collapse if more than one), (2) remove blank lines within the body, (3) remove trailing whitespace, (4) remove a trailing period from the subject, (5) remove leading whitespace from body lines.
- FR-013: A subject line longer than 72 characters is an error. Body line length is not linted.
- FR-011: `cxg lint <filepath>` reads and lints a file (for commit-msg hook use).
- FR-012: `cxg validate` is not provided. `cxg commit` is specified separately in `specs/commit/spec.md` and is outside this feature spec's implementation scope.
...
- `lint` command (linting + stdout passthrough)
- `lint --fix` flag (auto-fix + linting)
...
- Calling git commands from `lint` or the shared linting packages
